...
public class ReviewService {
    ...
    public List<ReviewDTO>  addReview(String username, String productId, ReviewRequest request) {
        ...
        if (existed.isPresent()) {
            ...
            Review review = p.getReviews().stream().filter(r ->  r.getReviewedBy().equals(user.get())).findFirst().get();
            ...
            if (null != review) {
                ...
                int totalPoints = p.getReviews().stream().map(Review::getRating).reduce(0, Integer::sum);
                int rating = totalPoints / p.getReviews().size();
